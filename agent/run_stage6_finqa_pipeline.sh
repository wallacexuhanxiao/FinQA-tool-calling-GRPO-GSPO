...
mkdir -p logs/agent outputs/agent/predictions outputs/agent/metrics outputs/finqa/predictions outputs/finqa/metrics data/agent/processed/stage6

python scripts/agent/prepare_agent_stage6_finqa_data.py 2>&1 | tee logs/agent/prepare_stage6_finqa_data.log

llamafactory-cli train configs/agent/agent_stage6_finqa_sft_r32.yaml 2>&1 | tee logs/agent/train_stage6_finqa_sft_r32.log
...
python scripts/agent/run_finqa_agent_controller.py \
  ...
  2>&1 | tee logs/agent/eval_stage6_finqa_agent_dev.log
