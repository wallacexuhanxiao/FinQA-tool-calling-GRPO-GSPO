...
log(){ echo "[$(date '+%F %T')] $*"; }
if [ ! -s data/finqa/processed/rag/generation/dense_bge_small_dev_top5_expanded.jsonl ]; then
  log "build expanded RAG input"
  python scripts/finqa/build_finqa_rag_generation_input_expanded.py \
    ...
    --top_k 5 --text_window 1 --table_window 1 --max_evidence_chunks 18
fi
log "infer RAG expanded dev with BASE Qwen2.5-7B-Instruct, no adapter"
PYTHONPATH=. python eval/infer_finqa_qwen.py \
  ...
  --output_jsonl outputs/finqa/rag/predictions/base_qwen25_7b_dense_bge_small_dev_top5_expanded.jsonl \
  --max_new_tokens 256
log "eval BASE RAG expanded dev"
python eval/eval_finqa_answer_exec.py \
  --pred_jsonl outputs/finqa/rag/predictions/base_qwen25_7b_dense_bge_small_dev_top5_expanded.jsonl \
  --out_metrics outputs/finqa/rag/metrics/base_qwen25_7b_dense_bge_small_dev_top5_expanded_answer_exec_metrics.json
log "done"
